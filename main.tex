...
\twocolumn[
  ...
  \begin{icmlauthorlist}
    ...
    \icmlauthor{Claas Voelker}{austin}
  ...
  \author{Evgenii Opryshko$^{1,2}$\thanks{Equal contribution}~\thanks{Correspondance to <eop@cs.toronto.edu>\quad Website: \href{https://ktolnos.github.io/ttgs/}{https://ktolnos.github.io/ttgs/}}, Junwei Quan$^{1}$\footnotemark[1], Claas Voelker$^{1,2}$, Yilun Du$^{3}$, Igor Gilitschenski$^{1,2}$
\\
...
\begin{table}[t]
  \centering
  \small
  \caption{\textbf{Success rates on OGBench manipulation tasks (GCIQL).} Improvements are at most marginal because evaluation goals lie outside the data manifold; TTGS correctly defaults to the base policy rather than hallucinating unsupported plans.}
...
Graph-based methods for GCRL have been explored along several axes, including action pruning from datasets~\citep{yin2024offlineimitationlearninggraph}, episode-graph RL in learned metric spaces~\citep{zhu2023valuememorygraphgraphstructured}, specialized distance learners~\citep{gas_baek2025, savinov2018semiparametrictopologicalmemorynavigation, zhang2021worldmodelgraphlearning}, and online refinement of value-derived edges~\citep{emmos2020sparsegraphical}. Closer to TTGS, \textbf{SoRB}~\citep{Eysenbach2019SoRB} operates online with a growing replay buffer, trains a distributional distance function, and applies hard edge pruning; \textbf{GSP}~\citep{lo2024gsp} learns four subgoal-conditioned models online for background planning via potential-based reward shaping; \textbf{IM-DSG}~\citep{bagaria2025imdsg} incrementally builds a skill graph through online novelty-driven exploration; and \textbf{Goal-conditioned Offline Planning from Curious Exploration}~\citep{bagatella2023goal}, closest in spirit to our work, performs offline graph-based subgoal planning over goal-conditioned values with a value-aggregation step against long-horizon noise. Relative to these, TTGS operates purely at test time on top of frozen offline GCRL value functions, with no additional training or online interaction; its soft-penalty mechanism preserves connectivity that hard thresholds would destroy (up to 99\% disconnection, see \autoref{apdx:hard_vs_soft}), and consistent gains across five base learners show that standard offline GCRL value functions already encode sufficient local geometric structure for planning without specialized distance learning.

\paragraph{Classical, motion, and task-and-motion planning.} TTGS shares the underlying structure of graph search with classical planning, motion planning, and TAMP, but differs in a key respect: edge costs come from learned, noisy value functions rather than known dynamics or collision checkers. This is what motivates the soft penalty, which has no direct analogue in settings where edge feasibility can be verified exactly.

Beyond graph search, several other planning paradigms have been proposed. Diffusion and generative models compose or augment trajectories for long-horizon planning~\citep{Janner2022Diffuser, Ajay2023DecisionDiffuser, Luo2025CompDiffuser, Zhang2025InferenceScaling, Lee2025SCoTS}, though they typically demand extra training and computation. Model-based planning uses learned dynamics for model-predictive control \citep{Williams2017MPPI, Chua2018PETS, Hafner2019PlaNet, Hansen2022TDMPC} or sequence-model planning with trajectory transformers \citep{Janner2021TrajectoryTransformer}. TTGS differs by providing fast ($<1$s planning time), training-free retrieval of the dataset states, which can complement generative and model-based methods by providing reliable subgoals.
...
\]
which involves no norm $\|\cdot\|$ and no state-space metric. The TD objective
\[
