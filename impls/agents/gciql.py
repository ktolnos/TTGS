...
class GCIQLAgent(flax.struct.PyTreeNode):
    ...
    def sample_values(self, observations, goals, seed=None):
        """
        Return a 'value-like' scalar in [-1/(1-γ), 0] so _value_distance can
        invert it to expected steps.

        If config['gc_negative'] is True (reward = -1 per step until goal), we
        pass through the raw value. If it's False (reward = +1 at/after goal),
        we shift by 1/(1-γ) so it fits the expected negative range.
        """
        raw_v = self.network.select('value')(observations, goals)
        gamma = self.config['discount']

        # Map to the canonical negative range expected by TTGS
        if self.config['gc_negative']:
            v = raw_v  # already in [-1/(1-γ), 0]
        else:
            # raw_v ≈ γ^T/(1-γ) ∈ [0, 1/(1-γ)]  => shift to negative range
            v = raw_v - (1.0 / (1.0 - gamma))  # => (γ^T - 1)/(1-γ) = -(1-γ^T)/(1-γ)
        return v
