...
class HIQLAgent(flax.struct.PyTreeNode):
    ...
    @jax.jit
    def sample_actions_low(self, observations, goals, seed=None, temperature=1.0):
        """Sample actions from the low-level actor."""
        dist = self.network.select('low_actor')(observations, goals, goal_encoded=False, temperature=temperature)
    ...
    def sample_values(self, observations, goals):
        """Sample values from the value network."""
