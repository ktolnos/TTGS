...
class QRLAgent(flax.struct.PyTreeNode):
    ...
    @jax.jit
    def sample_values(self, observations, goals, step_scale: float = 1.0):
        """
        Return a 'value-like' signal in [-1/(1-gamma), 0] so that the generic
        geometric inversion in _value_distance maps it back to expected steps.

        For QRL, the value head predicts a non-negative quasimetric distance d(s,g)
        with ~1 for a 1-step transition. We convert:
            v = -(1 - gamma ** (d / step_scale)) / (1 - gamma)

        Args:
            observations: [..., obs_dim]
            goals:        [..., obs_dim]
            step_scale:   per-step distance scale. Use 1.0 for QRL (default).

        Returns:
            values in [-1/(1-gamma), 0], shape-broadcasting with inputs.
        """
        d = self.network.select("value")(observations, goals)  # predicted distance >= 0
        d = jnp.maximum(d, 0.0)
        gamma = self.config["discount"]
        v = -(1.0 - jnp.power(gamma, d / step_scale)) / (1.0 - gamma)

        # Keep strictly inside the open interval for numerical stability in the log inversion
        eps = 1e-6
        v = jnp.clip(v, -1.0 / (1.0 - gamma) + eps, -eps)
        return v

    @jax.jit
    def sample_distance(self, observations, goals, step_scale: float = 1.0):
        """
        Return the agent's direct 'distance' estimate as expected steps.

        For plain QRL, d(s,g) is already in ~'steps'. If training shifts the scale,
        divide by step_scale to re-normalize 1 step ≈ 1.
        """
        d = self.network.select("value")(observations, goals)
        return jnp.maximum(d / step_scale, 0.0)
