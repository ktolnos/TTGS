...
class RISAgent(flax.struct.PyTreeNode):
    ...
    def waypoint_loss(self, batch, grad_params, rng=None):
        subgoal_dist = self.network.select("high_actor")(
            batch["observations"], batch["high_actor_goals"]
        )
        ...
        if self.config["use_subgoal_rep"]:
            ...
            wv1, wv2 = self.network.select("value")(
                subgoal_priors, batch["high_actor_goals"]
            )
            v = (v1 + v2) / 2
            wv = (wv1 + wv2) / 2
            wadv = wv - v
        ...
        w_dist = self.network.select("low_actor")(
            batch["observations"], subgoal_priors, goal_encoded=True
        )
        dist = self.network.select("actor")(
            batch["observations"], batch["high_actor_goals"], params=grad_params
        )
    ...
    def create(
        ...
        high_actor_def = GCActor(
            hidden_dims=config["actor_hidden_dims"],
            action_dim=(
                config["rep_dim"] if config["use_subgoal_rep"] else ex_goals.shape[-1]
            ),
            state_dependent_std=False,
            const_std=config["const_std"],
