...
def main(_):
    ...
    for ep_idx in trange(num_train_episodes + num_val_episodes):
        if FLAGS.dataset_type in ["path", "navigate", "explore"]:
            ...
            goal_ij = (
                adj_cells[np.random.randint(len(adj_cells))]
                if len(adj_cells) > 0
                else init_ij
            )
        else:
        ...
        ob, _ = env.reset(
            options=dict(task_info=dict(init_ij=init_ij, goal_ij=goal_ij))
        )
        ...
        while not done:
            if FLAGS.dataset_type == "explore":
                ...
                if step % 10 == 0:
                    ...
                    cur_subgoal_dir = cur_subgoal_dir / (
                        np.linalg.norm(cur_subgoal_dir) + 1e-6
                    )
