...
flags.DEFINE_string(
    "loco_restore_path", "experts/ant", "Locomotion agent restore path."
)
...
flags.DEFINE_integer(
    "max_episode_steps", 1001, "Maximum number of steps in an episode."
)
...
def main(_):
    ...
    loco_agent = load_agent(
        FLAGS.loco_restore_path, FLAGS.loco_restore_epoch, ob_dim, action_dim
    )
    ball_agent = load_agent(
        FLAGS.ball_restore_path, FLAGS.ball_restore_epoch, ob_dim, action_dim
    )
    ...
    def get_ball_action(ob, ball_xy, goal_xy):
        ...
        if "arena" in FLAGS.env_name:
            if np.linalg.norm(goal_xy - ball_xy) > 10:
                ...
                goal_xy = ball_xy + 10 * (goal_xy - ball_xy) / np.linalg.norm(
                    goal_xy - ball_xy
                )
        else:
            # In the actual maze environment, replace the goal with the oracle subgoal.
        ...
        agent_ob = np.concatenate(
            [ob[2:15], ob[17:], ball_xy - agent_xy, goal_xy - ball_xy]
        )
    ...
    for ep_idx in trange(num_train_episodes + num_val_episodes):
        if FLAGS.dataset_type == "navigate":
            ...
            agent_init_idx, ball_init_idx, goal_idx = np.random.choice(
                len(all_cells), 3, replace=False
            )
            ...
            agent_init_ij = all_cells[agent_init_idx]
            ball_init_ij = (
                all_cells[ball_init_idx] if cur_mode == "navigate" else agent_init_ij
            )
            goal_ij = all_cells[goal_idx]
        else:
        ...
        ob, _ = env.reset(
            options=dict(
                task_info=dict(
                    agent_init_ij=agent_init_ij,
                    ball_init_ij=ball_init_ij,
                    goal_ij=goal_ij,
                )
            )
        )
        ...
        virtual_agent_goal_xy = (
            None  # Virtual goal for the agent to move to when stuck.
        )
        ...
        while not done:
            ...
            if (
                virtual_agent_goal_xy is not None
                and np.linalg.norm(virtual_agent_goal_xy - next_ob[:2]) <= 0.5
            ):
            ...
            if FLAGS.dataset_type == "navigate":
                ...
                if (
                    ...
                    and np.linalg.norm(
                        np.array(dataset["observations"][-150:])[:, :2] - next_ob[:2],
                        axis=1,
                    ).max()
                    <= 2
                ):
                    ...
                    virtual_agent_goal_xy = np.array(
                        env.unwrapped.ij_to_xy(virtual_agent_goal_ij)
                    )
    ...
    train_dataset = {
        k: np.array(
            v[:total_train_steps], dtype=np.float32 if k != "terminals" else bool
        )
