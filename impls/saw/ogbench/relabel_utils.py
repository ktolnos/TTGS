...
def relabel_dataset(env_name, env, dataset):
    ...
    """
    assert (
        env.unwrapped._reward_task_id is not None
    ), "The environment is not in the single-task mode."
            ...
            for i in range(num_cubes):
                cube_xyzs_list.append(
                    dataset["qpos"][
                        :,
                        qpos_obj_start_idx
                        + i * qpos_cube_length : qpos_obj_start_idx
                        + i * qpos_cube_length
                        + 3,
                    ]
                )
...
def add_oracle_reps(env_name, env, dataset):
    ...
    if "maze" in env_name or "soccer" in env_name:
        ...
        if "maze" in env_name:
            ...
            oracle_reps = dataset["qpos"][
                :, qpos_ball_start_idx : qpos_ball_start_idx + 2
            ]
    elif "cube" in env_name or "scene" in env_name or "puzzle" in env_name:
        # Manipulation environments.
        qpos_obj_start_idx = 14
        ...
        if "cube" in env_name:
            ...
            cube_xyzs = np.stack(cube_xyzs_list, axis=1)
            oracle_reps = ((cube_xyzs - xyz_center) * xyz_scaler).reshape(
                -1, num_cubes * 3
            )
        elif "scene" in env_name:
            num_cubes = env.unwrapped._num_cubes
            ...
            cube_xyzs = np.stack(cube_xyzs_list, axis=1)
            cube_reps = ((cube_xyzs - xyz_center) * xyz_scaler).reshape(
                -1, num_cubes * 3
            )
            ...
            oracle_reps = np.concatenate(
                [cube_reps, button_reps, drawer_reps, window_reps], axis=-1
            )
        elif "puzzle" in env_name:
