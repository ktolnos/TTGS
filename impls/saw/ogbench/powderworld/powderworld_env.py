...
class PowderworldEnv(gymnasium.Env):
    ...
    def __init__(
        ...
        self._elems = [
            pw_element_names.index(elem_name) for elem_name in self._elem_names
        ]
        ...
        self.observation_space = Box(
            low=0,
            high=255,
            shape=(self._world_size, self._world_size, 6),
            dtype=np.uint8,
        )
    ...
    def set_tasks(self):
        ...
        if self._num_elems == 2:
            ...
            self.task_infos.append(
                dict(task_name="task1_plant", action_seq=action_seq, tol=32)
            )
            ...
            self.task_infos.append(
                dict(task_name="task2_stone", action_seq=action_seq, tol=32)
            )
            ...
            self.task_infos.append(
                dict(task_name="task3_square", action_seq=action_seq, tol=32)
            )
            ...
            self.task_infos.append(
                dict(task_name="task4_four_squares", action_seq=action_seq, tol=32)
            )
            ...
            for y in reversed(range(8)):
                for x in range(8):
                    if (x + y) % 2 == 0:
                        action_seq.append(("stone", x, y))
            self.task_infos.append(
                dict(task_name="task5_mosaic", action_seq=action_seq, tol=32)
            )
            ...
            self.task_infos.append(
                dict(task_name="task1_squares", action_seq=action_seq, tol=32)
            )
            ...
            self.task_infos.append(
                dict(task_name="task2_water_plant", action_seq=action_seq, tol=64)
            )
            ...
            self.task_infos.append(
                dict(task_name="task3_sandpile", action_seq=action_seq, tol=64)
            )
            ...
            self.task_infos.append(
                dict(task_name="task4_two_rooms", action_seq=action_seq, tol=64)
            )
            ...
            self.task_infos.append(
                dict(task_name="task5_elements", action_seq=action_seq, tol=96)
            )
            ...
            for x in range(8):
                action_seq.append(("water", x, 7))
            self.task_infos.append(
                dict(task_name="task1_bubbles", action_seq=action_seq, tol=96)
            )
            ...
            for _ in range(3):
                for x in range(8):
                    action_seq.append(("fire", x, 0))
            self.task_infos.append(
                dict(task_name="task2_firework", action_seq=action_seq, tol=96)
            )
            ...
            self.task_infos.append(
                dict(task_name="task3_three_rooms", action_seq=action_seq, tol=96)
            )
            ...
            for _ in range(10):
                add_square(action_seq, "plant", 4, 1, 3)
            self.task_infos.append(
                dict(task_name="task4_four_squares", action_seq=action_seq, tol=64)
            )
            ...
            self.task_infos.append(
                dict(task_name="task5_ice_plant", action_seq=action_seq, tol=96)
            )
        else:
            raise NotImplementedError
    ...
    def step(self, action):
        if self._action_step == 0:
            ...
            # Apply the action.
            np_action_world = np.zeros(
                (1, self._world_size, self._world_size), dtype=np.uint8
            )
            ...
            np_action_world[
                :,
                real_y : real_y + self._brush_size,
                real_x : real_x + self._brush_size,
            ] = elem
            world_delta = self.pw.np_to_pw(np_action_world)
            self._world = interp(
                ~self.pw.get_bool(world_delta, "empty")
                & ~self.pw.get_bool(self._world, "wall"),
                self._world,
                world_delta,
            )
    ...
    def _get_ob(self):
        ...
        if self._action_step == 1:
            ...
            color = self._elem_colors[self._action_elem_id]
            real_x = self._action_x * self._grid_size
            action_frame[:, real_x : real_x + self._brush_size, :] = (
                color * 255.0
            ).astype(np.uint8)
        return np.concatenate(
            [world_frame, action_frame], axis=2
