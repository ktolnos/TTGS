...
def interp2(switch_a, switch_b, if_false, if_a, if_b):
    return (
        ((~switch_a) & (~switch_b)) * if_false + (switch_a) * if_a + (switch_b) * if_b
    )
...
def conv2d(
    ...
    assert (
        c_in % groups == 0
    ), f"Number of input channels ({c_in}) not divisible by groups ({groups})."
    assert (
        c_out % groups == 0
    ), f"Number of output channels ({c_out}) not divisible by groups ({groups})."
...
class PWSim:
    def __init__(self):
        ...
        # ================ NUMPY KERNELS =================
        self.elem_vecs = {}
        self.elem_vecs_array = np.zeros(
            (self.NUM_ELEMENTS, self.NUM_CHANNEL), dtype=self.pw_type
        )
    ...
    def forward(self, world):
        # Helper Functions
        rand_movement = np.random.rand(
            world.shape[0], 1, world.shape[2], world.shape[3]
        ).astype(
            self.pw_type
        )  # For gravity
        rand_interact = np.random.rand(
            world.shape[0], 1, world.shape[2], world.shape[3]
        ).astype(
            self.pw_type
        )  # For gravity
...
class PWRenderer:
    ...
    def forward(self, world):
        ...
        if world.shape[1] > 1:
            ...
            img = (
                1 - velocity_field_display
            ) * img + velocity_field_display * velocity_field_colors
        img = np.clip(img, 0, 1)
        return img
...
class BehaviorGravity:
    ...
    def forward(self, world, info):
        ...
        world[:, 8:9] = interp(
            switch=(world[:, 2:3] == 1), if_false=world[:, 8:9], if_true=self.pw.zero
        )
        ...
        world[:] = interp2(
            switch_a=does_become_below_real,
            switch_b=does_become_above_real,
            if_false=world,
            if_a=below,
            if_b=above,
        )
        world[:, 8:9] = interp(
            switch=does_become_above_real, if_false=world[:, 8:9], if_true=self.pw.one
        )
    ...
    def forward(self, world, info):
        ...
        for fallLeft in [True, False]:
            ...
            is_above_right_density_higher = (
                world_above_right[:, 1:2] - world[:, 1:2]
            ) > 0
            is_gravity = world[:, 2:3] == 1
            is_below_left_gravity = world_below_left[:, 2:3] == 1
            is_above_right_gravity = world_above_right[:, 2:3] == 1
            is_element = self.pw.get_bool(world, "sand") | self.pw.get_bool(
                world, "dust"
            )
            is_above_right_element = self.pw.get_bool(
                world_above_right, "sand"
            ) | self.pw.get_bool(world_above_right, "dust")
...
class BehaviorFluidFlow:
    ...
    def forward(self, world, info):
        rand_movement, rand_interact, rand_element = info
        new_fluid_momentum = np.zeros(
            (world.shape[0], 1, world.shape[2], world.shape[3]), dtype=self.pw.pw_type
        )
        for fallLeft in [True, False]:
            get_in_dir = get_left if fallLeft else get_right
            get_in_not_dir = get_right if fallLeft else get_left
            ...
            is_air_move = self.pw.get_bool(world, "agentKangaroo") | self.pw.get_bool(
                world, "agentLemming"
            )
            is_element = (
...
class BehaviorIce:
    ...
    def forward(self, world, info):
        ...
        ice_can_melt = (
            conv2d(ice_melting_neighbors, self.pw.neighbor_kernel, padding=1) > 1
        )
        does_turn_water = (
            self.pw.get_bool(world, "ice") & ice_can_melt & (ice_chance < 0.02)
        )
        world[:] = interp(
            switch=does_turn_water, if_false=world, if_true=self.pw.elem_vecs["water"]
        )
        return world
...
class BehaviorWater:
    ...
    def forward(self, world, info):
        ...
        water_can_freeze = (
            conv2d(self.pw.get_elem(world, "ice"), self.pw.neighbor_kernel, padding=1)
            >= 3
        )
        does_turn_ice = (
            self.pw.get_bool(world, "water") & water_can_freeze & (ice_chance < 0.05)
        )
        world[:] = interp(
            switch=does_turn_ice, if_false=world, if_true=self.pw.elem_vecs["ice"]
        )
        return world
...
class BehaviorFire:
    ...
    def forward(self, world, info):
        ...
        fire_and_lava = self.pw.get_elem(world, "fire") + self.pw.get_elem(
            world, "lava"
        )
        has_fire_neighbor = (
            conv2d(fire_and_lava, self.pw.neighbor_kernel, padding=1) > 0
        )
        ...
        does_burn_ice = (
            self.pw.get_bool(world, "ice") & (burn_chance < 0.2) & has_fire_neighbor
        )
        does_burn = (
            does_burn_wood
            | does_burn_plant
        ...
        world[:, 3:5] -= (
            30 * get_left(does_burn_dust & has_fire_neighbor) * self.pw.left
        )
        world[:, 3:5] -= 30 * get_above(does_burn_dust & has_fire_neighbor) * self.pw.up
        world[:, 3:5] -= (
            30 * get_below(does_burn_dust & has_fire_neighbor) * self.pw.down
        )
        world[:, 3:5] -= (
            30 * get_right(does_burn_dust & has_fire_neighbor) * self.pw.right
        )

        world[:] = interp(
            switch=does_burn, if_false=world, if_true=self.pw.elem_vecs["fire"]
        )
        world[:] = interp(
            switch=does_burn_ice, if_false=world, if_true=self.pw.elem_vecs["water"]
        )
        ...
        fire_with_burnable_neighbor = (
            conv2d(burnables, self.pw.neighbor_kernel, padding=1) * fire_and_lava
        )
        in_fire_range = conv2d(
            fire_with_burnable_neighbor + self.pw.get_elem(world, "lava"),
            self.pw.neighbor_kernel,
            padding=1,
        )
        does_burn_empty = (
            self.pw.get_bool(world, "empty") & (in_fire_range > 0) & (burn_chance < 0.3)
        )
        world[:] = interp(
            switch=does_burn_empty, if_false=world, if_true=self.pw.elem_vecs["fire"]
        )
        ...
        does_fire_turn_empty = (
            self.pw.get_bool(world, "fire")
            & (fire_chance < 0.4)
            & (has_burnable_neighbor == 0)
        )
        world[:] = interp(
            switch=does_fire_turn_empty,
            if_false=world,
            if_true=self.pw.elem_vecs["empty"],
        )
...
class BehaviorPlant:
    ...
    def forward(self, world, info):
        ...
        plant_counts = conv2d(
            self.pw.get_elem(world, "plant"), self.pw.neighbor_kernel, padding=1
        )
        does_plantgrow = self.pw.get_bool(world, "water") & (plant_chance < 0.05)
        does_plantgrow_plant = (
            does_plantgrow & (plant_counts <= 3) & (plant_counts >= 1)
        )
        does_plantgrow_empty = does_plantgrow & (plant_counts > 3)

        wood_ice_counts = conv2d(
            self.pw.get_elem(world, "ice") + self.pw.get_elem(world, "wood"),
            self.pw.neighbor_kernel,
            padding=1,
        )
...
class BehaviorLava:
    ...
    def forward(self, world, info):
        water_counts = conv2d(
            self.pw.get_elem(world, "water"), self.pw.neighbor_kernel, padding=1
        )
        does_turn_stone = (water_counts > 0) & self.pw.get_bool(world, "lava")
        world[:] = interp(
            switch=does_turn_stone, if_false=world, if_true=self.pw.elem_vecs["stone"]
        )

        lava_counts = conv2d(
            self.pw.get_elem(world, "lava"), self.pw.neighbor_kernel, padding=1
        )
...
class BehaviorAcid:
    ...
    def forward(self, world, info):
        ...
        does_acid_dissapear = (is_acid & acid_rand & get_below(is_block)) | (
            is_acid & acid_rand & get_above(is_block)
        )
        does_block_dissapear = (
            is_block & get_above(acid_rand) & get_above(is_acid)
        ) | (is_block & get_below(acid_rand) & get_below(is_acid))
        does_dissapear = does_acid_dissapear | does_block_dissapear
        world[:] = interp(
            switch=does_dissapear, if_false=world, if_true=self.pw.elem_vecs["gas"]
        )
        return world
...
class BehaviorCloner:
    ...
    def forward(self, world, info):
        ...
        for get_dir in [get_below, get_above, get_left, get_right]:
            is_cloner_empty = self.pw.get_bool(world, "cloner") & (
                (cloner_assigns == 0) | (cloner_assigns == 13)
            )
        ...
        # Cloner produce
        cloner_assigns_ids = np.clip(world[:, 6], 0, self.pw.NUM_ELEMENTS - 1).astype(
            int
        )
        ...
        for get_dir in [get_below, get_above, get_left, get_right]:
            ...
            world[:] = interp(
                switch=is_dir_cloner_not_empty,
                if_false=world,
                if_true=cloner_assigns_vec_dir,
            )
        return world
...
class BehaviorVelocity:
    ...
    def forward(self, world, info):
        ...
        for n in range(2):
            ...
            velocity_angle_int = np.remainder(
                np.floor(velocity_field_angle * 8 + 0.5), 8
            )
            is_velocity_enough = (
                velocity_field_magnitudes > (1.0 if n == 0 else 2.0)
            ) & (~self.pw.get_bool(world, "wall"))
            ...
            world[:] = interp_swaps8(
                swaps, world, dw[0], dw[1], dw[2], dw[3], dw[4], dw[5], dw[6], dw[7]
            )
        ...
        for i in range(1):
            velocity_field[:, 0:1] = (
                conv2d(velocity_field[:, 0:1], self.pw.neighbor_kernel / 18, padding=1)
                + velocity_field[:, 0:1] * 0.5
            )
            velocity_field[:, 1:2] = (
                conv2d(velocity_field[:, 1:2], self.pw.neighbor_kernel / 18, padding=1)
                + velocity_field[:, 1:2] * 0.5
            )
...
class BehaviorFish:
    ...
    def forward(self, world, info):
        ...
        for angle in [0, 1, 2, 3]:
            ...
            density = world[:, 1:2]
            is_empty_in_dir = self.pw.direction_func(
                angle * 2, is_gravity & (density <= 2)
            )
            ...
            does_become_fish = self.pw.direction_func(
                ((angle * 2) + 4) % 8, does_become_opposite
            )
...
class BehaviorBird:
    ...
    def forward(self, world, info):
        ...
        random_dirs = np.concatenate(
            [np.cos(rand_movement * np.pi * 2), np.sin(rand_movement * np.pi * 2)],
            axis=1,
        )
        ...
        vel_delta_flocking = 1 * conv2d(
            bird_vel * self.pw.get_elem(world, "agentBird"),
            self.flocking_kernel,
            padding=6,
        )

        bird_vel += self.pw.get_elem(world, "agentBird") * (
            vel_delta_obstacle + vel_delta_flocking
        )
        ...
        world[:, 6:8] = interp(
            self.pw.get_bool(world, "agentBird"), original_68, bird_vel
        )
...
class BehaviorKangaroo:
    ...
    def forward(self, world, info):
        ...
        kangaroo_jump_state = interp(
            switch=is_kangaroo_jump, if_false=kangaroo_jump_state, if_true=self.pw.one
        )
        kangaroo_jump_state = interp(
            switch=is_kangaroo,
            if_false=kangaroo_jump_state,
            if_true=kangaroo_jump_state - 0.1,
        )
...
class BehaviorMole:
    ...
    def forward(self, world, info):
        ...
        beetle_dir = interp(
            switch=does_beetle_dir_change, if_false=beetle_dir, if_true=new_beetle_dir
        )
        ...
        has_supports = conv2d(
            (density >= 3).astype(self.pw.pw_type), self.pw.neighbor_kernel, padding=1
        )
        world[:, 2:3] = (1 - is_beetle_num) * world[:, 2:3] + is_beetle_num * (
            has_supports < 2
        )
        ...
        for angle in [0, 1, 2, 3]:
            ...
            does_move_in_dir = (
                (rand_element < 0.5) & is_solid_in_dir & ~is_wall_in_dir
            ) | ((rand_element < 0.1) & ~is_wall_in_dir)
            does_become_opposite = (
                is_angle_match & does_move_in_dir & is_beetle & ~is_beetle_in_dor
            )
            does_become_beetle = self.pw.direction_func(
                ((angle * 2) + 4) % 8, does_become_opposite
            )

            resulting_world = interp(
                switch=is_solid_in_dir,
                if_false=dw[angle],
                if_true=self.pw.elem_vecs["empty"],
            )
...
class BehaviorLemming:
    ...
    def forward(self, world, info):
        ...
        for fallLeft in [True, False]:
            ...
            does_become_above = (
                is_element
                & is_forward_density_higher
                & is_above_density_lower
                & is_density_forward_above_lower
            )
...
class BehaviorSnake:
    ...
    def forward(self, world, info):
        ...
        does_become_trail = np.zeros(
            (world.shape[0], 1, world.shape[2], world.shape[3]), dtype=bool
        )
        does_become_snake = np.zeros(
            (world.shape[0], 1, world.shape[2], world.shape[3]), dtype=bool
        )
        dir_snake_came_from = np.zeros(
            (world.shape[0], 1, world.shape[2], world.shape[3]), dtype=self.pw.pw_type
        )
        ones = np.ones(
            (world.shape[0], 1, world.shape[2], world.shape[3]), dtype=self.pw.pw_type
        ...
        for angle in [0, 1, 2, 3]:
            ...
            does_become_this_angle = self.pw.direction_func(
                ((angle * 2) + 4) % 8, is_snake_angle
            ) & ~self.pw.get_bool(world, "wall")
            does_turn = does_turn | (
            ...
            world[:] = interp(
                (does_become_snake & does_become_this_angle),
                world,
                self.pw.elem_vecs["empty"],
            )
            does_become_snake = does_become_snake | does_become_this_angle
            dir_snake_came_from = interp(
                switch=does_become_this_angle,
                if_false=dir_snake_came_from,
                if_true=ones * angle,
            )

            # Perform Swaps
        acid_or_not = interp(
            rand_element < 0.05, self.pw.elem_vecs["empty"], self.pw.elem_vecs["acid"]
        )
        snake_trail = interp(
            old_snake_energy > 0, acid_or_not, self.pw.elem_vecs["agentSnake"]
        )
        world[:] = interp(does_become_trail, if_false=world, if_true=snake_trail)
        world[:] = interp(
            does_become_snake, if_false=world, if_true=self.pw.elem_vecs["agentSnake"]
        )

        # This is where I am about to turn in.
        turned_dir_came_from = (
            dir_snake_came_from + 1 - 2 * (rand_element < 0.5).astype(int)
        ) % 4
        ...
        does_turn = (
            does_turn
            & ~self.pw.get_bool(in_dir, "agentSnake")
            & ~self.pw.get_bool(in_dir, "wall")
        )

        is_snake = self.pw.get_bool(world, "agentSnake")
        dir_snake_came_from = interp(
            switch=does_turn, if_false=dir_snake_came_from, if_true=turned_dir_came_from
        )
        new_snake_dir = interp(
            switch=(is_snake & ~was_snake),
            if_false=old_snake_dir,
            if_true=dir_snake_came_from,
        )
